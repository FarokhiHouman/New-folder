...
// Type: Slb.Production.Engineering.Views.Well.TubingPlugViewModel
...
public class TubingPlugViewModel : DomainObjectPropertyBagViewModel<TubingPlug>
{
  public TubingPlugViewModel(TubingPlug model)
    : base(model)
  {
    DownholeLocation location = model.Location as DownholeLocation;
    if (location != (DownholeLocation) null)
      this.LocationViewModel = this.RegisterContent<DownholeEquipmentLocationViewModel>(new DownholeEquipmentLocationViewModel(location, (NamedItem) model));
  ...
  public override ImageSource Image => ImageKey.TubingPlug.ToImageSource(ImageSizes.Image16);
